###############################################################################
# File: generate_cifar_dataset.py
#
# Purpose:
#   Generate a reusable dataset of individual CIFAR-10-sized samples from a
#   pretrained Energy Matching model. Generation is still batched on GPU; only
#   the final output is saved as individual PNG files for later FID/KID reuse.
#
# Example:
...
def write_metadata(run_dir, device):
    metadata = {
        "script": "experiments/cifar10/generate_cifar_dataset.py",
...
def main(_):
    ...
    run_name = datetime.now().strftime("cifar10_em_%Y%m%d_%H%M%S")
